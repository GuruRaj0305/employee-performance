  ...
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      if (!table.performance_cycle_id) {
        const [cycles] = await queryInterface.sequelize.query("SELECT id FROM performance_cycles WHERE name = :name LIMIT 1", {
        ...
        await queryInterface.sequelize.query("UPDATE review_sessions SET performance_cycle_id = :performanceCycleId WHERE performance_cycle_id IS NULL", {
